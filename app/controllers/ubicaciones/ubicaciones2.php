...
class UbicacionController
{
  ...
  // Obtener municipios filtrados con paginación - VERSIÓN CORREGIDA
public function obtenerMunicipiosFiltrados($id_estado = null, $busqueda = '', $offset = 0, $limit = 20)
{
  try {
    $sql = "SELECT m.*, e.nom_estado 
            FROM municipios m
    ...
    if (!empty($busqueda)) {
      $sql .= " AND m.nom_municipio LIKE ?";
      $params[] = "%" . $busqueda . "%";
    }
    
    $sql .= " ORDER BY m.nom_municipio ASC";
    
    // Agregar LIMIT directamente al SQL (no como parámetro)
    ...
    throw new Exception("Error al obtener municipios filtrados: " . $e->getMessage());
  }
}

// Contar municipios filtrados - VERSIÓN CORREGIDA
public function contarMunicipiosFiltrados($id_estado = null, $busqueda = '')
{
  try {
    ...
    throw new Exception("Error al contar municipios filtrados: " . $e->getMessage());
  }
}
